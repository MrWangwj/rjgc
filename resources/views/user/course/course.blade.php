...
<head>
    ...
    <title>我的课表</title>
    <link rel="stylesheet" href="/unit/Swiper-4.0.7/dist/css/swiper.min.css">
...
<nav class="title">
    ...
    <div class="week-day">
        ...
        <label onclick="javascript:location='/user/course/course/add'">
            <span>+</span>
        </label>
...
<div class="content">
    ...
    @foreach($user->courses as $course)
        <label
                ...
                data-start-week="{{ $course->week_start }}"
                data-end-week="{{ $course->week_end }}"
                data-start-section="{{ $course->section_start }}"
                data-end-section="{{ $course->section_end }}"
                data-week-day="{{ $course->week_day }}"
                data-status="{{ $course->week_type }}"
        >
...
<script src="/unit/Swiper-4.0.7/dist/js/swiper.min.js"></script>
<script>
    $.ajaxSetup({
        headers: {
...
<script>
    ...
    $(document).ready(function () {
        ...
        var startSchool = '{{ $startSchool }}'; // 获取开学时间
    ...
    $('#coverflow').on('click', '.swiper-slide', function (e) {
        e.stopPropagation();
    });
            ...
            if(option.weekDay == options.weekDay){
                if((Math.max(option.startSection, options.startSection) <
                    ...
                    var slide = '<div class="swiper-slide '+courseClass+'">' +
                        '<p style="text-align: right"><a href="/user/course/course/edit/'+ options.id +'">编辑</a>|<span onclick="deleteCourse('+options.id+')">删除</span></p>'+
                        '<h4>'+ options.name + '</h4>' +
...
//        $('#month').text(month);
    ...
    function deleteCourse(id) {
        if(window.confirm("确认要删除这门课程吗？")){
            $.post(
                '/user/course/course/delete',
                {
                    id: id,
                },
                function (data) {
